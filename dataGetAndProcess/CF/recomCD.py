...
'''
@File   :   recomCD.py
@Time   :   2022/06/28 11:00:13
@Author :   ferry-hhh
@Desc   :   跨领域推荐算法，借助UserCF思想，找出相似的用户这个中间桥梁，推荐相似用户喜爱的其他类物品给目标用户
'''
...
class RecomCD():
    def __init__(self, type, filename):
        ...
        self.type = ANIME if type == None else type
        self.user_item_name = "anime_user_item.json"
        self.user_sim_name = "anime_user_sim.json"
        self.user_item_o1_name = "comic_user_item.json"
        self.user_item_o2_name = "novel_user_item.json"
        if self.type == ANIME:
            self.user_sim_name = "anime_user_sim.json"
            self.user_item_name = "anime_user_item.json"
            self.user_item_o1_name = "comic_user_item.json"
            self.user_item_o2_name = "novel_user_item.json"
        elif self.type == COMIC:
            self.user_sim_name = "comic_user_sim.json"
            self.user_item_name = "comic_user_item.json"
            self.user_item_o1_name = "anime_user_item.json"
            self.user_item_o2_name = "novel_user_item.json"
        elif self.type == NOVEL:
            self.user_sim_name = "novel_user_sim.json"
            self.user_item_name = "novel_user_item.json"
            self.user_item_o2_name = "anime_user_item.json"
            self.user_item_o1_name = "comic_user_item.json"
        ...
        try:
            ...
            with open(os.path.join(DATA_PATH, self.user_item_o1_name), "r") as f3:
                self.user_item_o1 = json.load(f3)
            with open(os.path.join(DATA_PATH, self.user_item_o2_name), "r") as f4:
                self.user_item_o2 = json.load(f4)
    ...
    def user_rec_CD(self, user):
        ...
        for v, Wuv in sorted_users:
            for item in self.user_item_o1[v]:
                # if item in watched_items:continue
                rank.setdefault(item, 0)
                rank[item] += Wuv * float(self.user_item_o1[v][item])
        res1 = sorted(rank.items(), key=itemgetter(1), reverse=True)[:REC_ITEMS]
        print('推荐12部漫画：', res1)
        for v, Wuv in sorted_users:
            for item in self.user_item_o2[v]:
                # if item in watched_items:continue
                rank.setdefault(item, 0)
                rank[item] += Wuv * float(self.user_item_o2[v][item])
        res2 = sorted(rank.items(), key=itemgetter(1), reverse=True)[:REC_ITEMS]
        print('推荐12部小说：', res2)
        return res1, res2
